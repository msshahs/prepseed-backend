...
let redisCacheGlobal = null;
const createClient = (...args) => {
	redisCacheGlobal = Redis(...args);
	redisCacheGlobal.on('error', (er) => {
		console.log('Failed to connect to redis server');
		console.error(er);
	});
	redisCacheGlobal.on('connect', () => {
		// console.log('connected to redis');
	});
	return redisCacheGlobal;
};

const redisStore = (...args) => {
	const redisCache = createClient(...args);
	// const storeArgs = redisCache.options;
	const defaultTTL = args[0].ttl;
	const isCacheableValue = args[0].isCacheableValue;
	...
	return {
		...
		set: function (key, value, options, cb) {
			...
			return new Promise((resolve, reject) => {
				...
				const ttl = options.ttl || options.ttl === 0 ? options.ttl : defaultTTL;
		...
		mget: (...args) =>
			new Promise((resolve, reject) => {
				...
				redisCache.mget.apply(redisCache, [
					...args,
					handleResponse(cb, { parse: true }),
				]);
			}),
		del: (...args) =>
			new Promise((resolve, reject) => {
